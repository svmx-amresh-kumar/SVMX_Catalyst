...
public class SCN_Checklist_1_RS_10577 extends BaseLib {
	...
	public void prerequisites() throws Exception {
		...
		if (BaseLib.sOSName.equalsIgnoreCase("ios")) {
			commonUtility.addJiraLinkInExtentReport("https://servicemax.atlassian.net/browse/RS-10514");
		} else {
	...
	public void RS_10577() throws Exception {
		...
		Assert.assertTrue(sTextAns.equals(sCityPrefill),
				"Text question not Prefilled--expected: " + sCityPrefill + " actual: " + sTextAns + "");
		ExtentManager.logger.log(Status.PASS,
				"Text question prefilled sucessfully expected: " + sCityPrefill + " actual: " + sTextAns + "");

		sPicklistAns = checklistPo.geteleChecklistAnsPicklist(sPicklistq).getAttribute("value");
		Assert.assertTrue(sPicklistAns.equals(sOrderStatusPrefill),
				"Picklist question not Prefilled.--expected: " + sOrderStatusPrefill + " actual: " + sPicklistAns + "");
		ExtentManager.logger.log(Status.PASS, "Picklist question prefilled sucessfully--expected: "
				+ sOrderStatusPrefill + " actual: " + sPicklistAns + "");

		sNumberAns = checklistPo.geteleChecklistAnsNumber(sNumberq).getAttribute("value");
		Assert.assertTrue(sNumberAns.equals(sIdleTimePrefill),
				"Number question not Prefilled.--expected: " + sIdleTimePrefill + " actual: " + sNumberAns + "");
		ExtentManager.logger.log(Status.PASS,
				"Number question prefilled sucessfully--expected: " + sIdleTimePrefill + " actual: " + sNumberAns + "");

		/*
		 * sDateAns = checklistPo.geteleChecklistAnsDate(sDateq).getAttribute("value");
		 * Assert.assertTrue(sDateAns.equals(sScheduleddatePrefill),
		 * "Date question not Prefilled.");
		 * ExtentManager.logger.log(Status.PASS,"Date question prefilled sucessfully");
		 * 
		 * sDatetimeAns =
		 * checklistPo.geteleChecklistAnsDate(sDatetimeq).getAttribute("value");
		 * System.out.println(sDatetimeAns);
		 * Assert.assertTrue(sDatetimeAns.equals(sScheduledDateTimePrefill),
		 * "DateTime question not Prefilled."); ExtentManager.logger.log(Status.
		 * PASS,"DateTime question prefilled sucessfully");
		 */
		...
		Assert.assertEquals(workOrderPo.getEleBillingTypeLst().getAttribute("value"), sBillingTypeSOU,
				"Picklist Source Object is not updated--Expected: " + sBillingTypeSOU + " Actual : " + sBillingType
						+ "");
		ExtentManager.logger.log(Status.PASS, "Source Object Update for Picklist Sucessfull Expected : "
				+ sBillingTypeSOU + " Actual: " + sBillingType + "");
		...
		Assert.assertEquals(workOrderPo.GetEleNoOfTimesAssigned_Edit_Input().getAttribute("value"),
				sNooftimesAssignedSOU, "Number Source Object is not updated--Expected:" + sNooftimesAssignedSOU
						+ " Actual : " + sNoOfTimesAssigned + "");
		 ...
		 * workOrderPo.getEleScheduledDateTimeTxt().getAttribute("value");
		 * System.out.println(sScheduledDateTime);
		 * Assert.assertEquals(workOrderPo.getEleScheduledDateTimeTxt().getAttribute(
		 * "value"), sScheduledDateTimeSou, "DateTime Source Object is not updated");
		 * ExtentManager.logger.log(Status.
		 * PASS,"Source Object Update for DateTime Sucessfull");
		 */
		 ...
		 * Assert.assertEquals(workOrderPo.getEleScheduledDateLst().getAttribute("value"
		 * ), sScheduledDateSOU, "Date Source Object is not updated");
		 * ExtentManager.logger.log(Status.
		 * PASS,"Source Object Update for Date with function Today Sucessfull");
		 */

		// 5. Text
		sProformaInvoice = workOrderPo.getEleProformaInvoiceTxt().getAttribute("value");
		Assert.assertEquals(workOrderPo.getEleProformaInvoiceTxt().getAttribute("value"), sProformaInvoiceSOU,
		...
		/*
		 * SimpleDateFormat parser = new SimpleDateFormat("MM/dd/yy"); Date dTempDate1 =
		 * parser.parse(sScheduleddatePrefill); SimpleDateFormat formatter = new
		 * SimpleDateFormat("yyyy-MM-dd"); String sformattedDate =
		 * formatter.format(dTempDate1);
		 * 
		 * Assert.assertTrue(ChecklistAnsjson.contains(sformattedDate),
		 * "checklist date answer was not sycned to server in checklist answer");
		 * ExtentManager.logger.log(Status.
		 * PASS,"checklist date question answer synced to server");
		 * 
		 * SimpleDateFormat parser1 = new SimpleDateFormat("MM/dd/yy hh:mm"); dTempDate1
		 * = parser1.parse(sScheduledDateTime); SimpleDateFormat formatter1 = new
		 * SimpleDateFormat("yyyy-MM-dd HH:mm"); String stempDate =
		 * formatter1.format(dTempDate1);
		 * System.out.println("formatter1.format value   "+stempDate); dTempDate1 =
		 * formatter1.parse(stempDate); //adding 7 hours to set to UTC/GMT time.. this
		 * is from PST timezone as Instant insDate =dTempDate1.toInstant().plus(7,
		 * ChronoUnit.HOURS); System.out.println("7 aded to instant"+insDate);
		 * 
		 * //formatter1.format(datetime1);
		 * //System.out.println("after format datetime1"+datetime1); String
		 * sformattedDatetime = formatter1.format(dTempDate1); dTempDate1 =
		 * Date.from(insDate); sformattedDatetime = formatter1.format((dTempDate1));
		 * 
		 * Assert.assertTrue(ChecklistAnsjson.contains(sformattedDatetime),
		 * "checklist datetime answer was not sycned to server in checklist answer");
		 * ExtentManager.logger.log(Status.
		 * PASS,"checklist datetime question answer synced to server");
		 */
