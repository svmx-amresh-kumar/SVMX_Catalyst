...
public class SCN_Mapping_RS_10556 extends BaseLib {
	...
	public void RS_10556() throws Exception {
		...
		if (BaseLib.sOSName.equalsIgnoreCase("ios")) {
			commonUtility.addJiraLinkInExtentReport("https://servicemax.atlassian.net/browse/RS-10556");
		} else {
			commonUtility.addJiraLinkInExtentReport("https://servicemax.atlassian.net/browse/RS-12084");
	...
	//creating one hr event
		...
		JSONArray sJsonArrayWO1 = restServices.restGetSoqlJsonArray(
				"Select+SVMXC__Order_Status__c,+SVMXC__Order_Type__c+from+SVMXC__Service_Order__c+where+SVMXC__Service_Order__c.name=\'"
						+ sworkordernumber + "\'");
		// String sordertype = restServices.getJsonValue(sJsonArrayWO1,
		...
		System.out.println(
				":):):):):):):):):before save header):):):):):):):)before saveheader:):):):):):):):):):):):)before save:):):):):):):)");
		...
		try {
			Assert.assertEquals(fetchisBillable, true);
		...
		ExtentManager.logger.log(Status.PASS,
				"Mapping validation is successful. Covered fields for Header record - lookup, record type, checkbox, picklist, number, currency, text, date, datetime, Email, Url and Phone. Also using these literals - SVMX.USERTRUNK,SVMX.CURRENTUSERID,SVMX.CURRENTRECORD");

		System.out.println("Validation of child after data sync ");

		// commonsUtility.tap(workOrderPo.geteletaponfirstpart(),20,20);
		commonUtility.tap(workOrderPo.geteletaponfirstpart(), 30, 36);
		Thread.sleep(2000);

		// Verifying mapping After data sync on child
		System.out.println(
				":):):):):):):):): After Datasync child):):):):):):):)After Datasync child:):):):):):):):):):):):)After Datasync child:):):):):):):)");
		...
		try {
			Assert.assertEquals(fetchisBillable, true);
			ExtentManager.logger.log(Status.PASS, "Billable value mapped Successful ");
		} catch (AssertionError e) {
			System.out.println(e);
